...
int __FASTERXML_VERSION_1_1_5 = 0 ;
	...
	{								\
		...
		{							\
			if( (unsigned char)*(_base_) > 127 )		\
				continue;				\
			if( CDATA_flag == 0 && (_base_)[0] == '<' && (_base_)[1] != '!' && (isalpha((_base_)[1])||(_base_)[1]=='_'||(_base_)[1]=='/') )	\
			{						\
...
#if _DEBUG
printf( "%03dTOKENXML - '<' or '<!' or '<?' - tag[%d] begin[%.*s]\n" , __LINE__ , tag , len , begin );
#endif

			__FASTERXML_RECCOUNT++;
			if(( __FASTERXML_RECCOUNT == 1 ) && (tag != FASTERXML_TOKEN_LAB) && (tag != FASTERXML_TOKEN_LAB_SP))
				return FASTERXML_ERROR_XML_INVALID;

			else if( tag == FASTERXML_TOKEN_LAB || tag == FASTERXML_TOKEN_LAB_SP )
				break;
...
#if _DEBUG
printf( "%03dTOKENXML - '/' or '?' - tag[%d] begin[%.*s]\n" , __LINE__ , tag , len , begin );
#endif
...
_PREREAD_GO :
		...
		for( i = 0 ; i < g_nSkipXmlTagCount ; i++ )
		{
			if( node_len == g_nSkipXmlTagsLen[i] && STRNICMP( node , == , g_pcSkipXmlTags[i] , node_len ) )
				break;
		}
		if( i < g_nSkipXmlTagCount )
			continue;
...
#if _DEBUG
printf( "%03dTOKENCONTENT - TEXT - tag[%d] begin[%.*s]\n" , __LINE__ , tag , len , begin );
#endif
		content = begin ;
		content_len = len ;
		...
		TOKENXML(*xml_ptr,begin,len,tag,FASTERXML_ERROR_END_OF_BUFFER)
#if _DEBUG
printf( "%03dTOKENXML - '<' - tag[%d] begin[%.*s]\n" , __LINE__ , tag , len , begin );
#endif
		...
		TOKENXML(*xml_ptr,begin,len,tag,FASTERXML_ERROR_END_OF_BUFFER)
#if _DEBUG
printf( "%03dTOKENXML - '/' or '?' or other - tag[%d] begin[%.*s]\n" , __LINE__ , tag , len , begin );
#endif
		if( tag == FASTERXML_TOKEN_SLASH )
		{
			TOKENXML(*xml_ptr,begin,len,tag,FASTERXML_ERROR_END_OF_BUFFER)
#if _DEBUG
printf( "%03dTOKENXML - close[%.*s] - tag[%d] begin[%.*s]\n" , __LINE__ , node_len , node , tag , len , begin );
#endif
			...
			TOKENXML(*xml_ptr,begin,len,tag,FASTERXML_ERROR_END_OF_BUFFER)
#if _DEBUG
printf( "%03dTOKENXML - not '>' - tag[%d] begin[%.*s]\n" , __LINE__ , tag , len , begin );
#endif
		    ...
		    , void *p )
{
	...
	nret = _TravelXmlBuffer( & xml_ptr , xpath , 0 , xpath_size
				...
				, pfuncCallbackOnXmlNode
				, p , NULL , 0 ) ;
	if( nret == FASTERXML_INFO_END_OF_BUFFER  || nret == 0  )
		return 0;
	else  
		return nret;
