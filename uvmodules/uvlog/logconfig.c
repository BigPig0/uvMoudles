...
consol_appender_t* create_appender_consol(){
    SAFE_MALLOC(consol_appender_t, ret);
    ret->filter = create_list(void*); //filter*
    list_init(ret->filter);
    return ret;
}

void destory_appender_consol(consol_appender_t* appender) {
}

file_appender_t* create_appender_file(){
    SAFE_MALLOC(file_appender_t, ret);
    ret->filter = create_list(void*); //filter*
    list_init(ret->filter);
    return ret;
}

void destory_appender_file(file_appender_t* appender) {
    if(appender->file_name)
        string_destroy(appender->file_name);
}

rolling_file_appender_t* create_appender_rolling_file(){
    SAFE_MALLOC(rolling_file_appender_t, ret);
    ret->filter = create_list(void*); //filter*
    list_init(ret->filter);
...
void destory_appender_rolling_file(rolling_file_appender_t* appender) {
...
void destory_appender(appender_t* appender) {
    ...
    if(appender->type == consol)
        destory_appender_consol((consol_appender_t*)appender);
    else if(appender->type == file)
        destory_appender_file((file_appender_t*)appender);
    else if(appender->type == rolling_file)
        destory_appender_rolling_file((rolling_file_appender_t*)appender);
...
logger_t* create_logger() {
    ...
    ret->additivity = true;
    ret->ring = create_ring_buff(sizeof(task_log_msg_t),100,NULL);
    return ret;
}
